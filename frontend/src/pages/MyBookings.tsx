...
const MyBookings = () => {
  ...
  const { upcoming, past, pendingApproval, pendingPayment } = useMemo(() => {
    const now = new Date();
    ...
    const upcoming = withDates.filter(b => b._checkOut >= now).sort((a,b) => a._checkIn.getTime() - b._checkIn.getTime());
    const past = withDates.filter(b => b._checkOut < now).sort((a,b) => b._checkOut.getTime() - a._checkOut.getTime());
    ...
    return { upcoming, past, pendingApproval, pendingPayment };
  }, [data]);
